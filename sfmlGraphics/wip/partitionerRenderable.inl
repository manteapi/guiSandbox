...
void PartitionerRenderable<PartitionData>::buildVoxelTriangles()
{
    for(const Voxel<PartitionData>& v : m_partitioner->m_image)
    {
        for(const FacetHandle & f : v.triangles)
        {
            ...
            while(++eit != eBegin);
            glm::vec3 normal = glm::normalize(glm::cross(triangle[0]-triangle[1], triangle[0]-triangle[2]));
            for(size_t i=0; i<3; ++i)
            {
                m_voxelTriColors.push_back( glm::vec4(1.0,1.0,1.0,1.0) );
                m_voxelTriPositions.push_back( triangle[i] );
                m_voxelTriNormals.push_back( normal );
            }
        }
    }
...
void PartitionerRenderable<PartitionData>::buildGrid()
{
    ...
    for(size_t i=0; i<ySlice; ++i)
    {
        for(size_t j=0; j<zSlice; ++j)
        {
            ...
            m_gridPositions.push_back( glm::vec3(minBB[0], minBB[1]+yOffset, minBB[2]+zOffset) );
            m_gridPositions.push_back( glm::vec3(maxBB[0], minBB[1]+yOffset, minBB[2]+zOffset) );
    ...
    for(size_t i=0; i<xSlice; ++i)
    {
        for(size_t j=0; j<ySlice; ++j)
        {
            ...
            m_gridPositions.push_back( glm::vec3(minBB[0]+xOffset, minBB[1]+yOffset, minBB[2]) );
            m_gridPositions.push_back( glm::vec3(minBB[0]+xOffset, minBB[1]+yOffset, maxBB[2]) );
            m_gridColors.push_back( glm::vec4(0.0,0.0,0.0,1.0) );
            m_gridColors.push_back( glm::vec4(0.0,0.0,0.0,1.0) );
        }
    }
    for(size_t i=0; i<xSlice; ++i)
    {
        for(size_t j=0; j<zSlice; ++j)
        {
            float xOffset = (i/float(ySlice-1))*scale[0];
            float zOffset = (j/float(zSlice-1))*scale[2];
            m_gridPositions.push_back( glm::vec3(minBB[0]+xOffset, minBB[1], minBB[2]+zOffset) );
            m_gridPositions.push_back( glm::vec3(minBB[0]+xOffset, maxBB[1], minBB[2]+zOffset) );
            m_gridColors.push_back( glm::vec4(0.0,0.0,0.0,1.0) );
            m_gridColors.push_back( glm::vec4(0.0,0.0,0.0,1.0) );
        }
    }
